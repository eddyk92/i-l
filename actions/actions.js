...
export function getStore(callback){
  ...
  Cosmic.getObjects(config, function(err, response){
    ...
    /* Exercise Items
    ======================== */
    let exercise_items = objects.type['exercise']
    exercise_items = _.sortBy(exercise_items, 'order')
    AppStore.data.exercise_items = exercise_items
...
export function getPageData(page_slug, post_slug){
  ...
  if(post_slug){
    ...
    if(page_slug === 'exercise'){
      const exercise_items = data.exercise_items
      const exercise_item = _.findWhere(exercise_items, { slug: post_slug })
      page.title = exercise_item.title
    }
  }
